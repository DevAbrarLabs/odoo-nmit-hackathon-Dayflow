    ...
    def do_GET(self):
        ...
        with connect() as db:
            ...
            if path=="/leave":
                ...
                trs="".join(f"<tr><td>{e(r['date_from'])} → {e(r['date_to'])}</td><td>{e(r['leave_type'])}</td><td>{e(r['reason'])}</td><td><span class='status {r['status'].lower()}'>{e(r['status'])}</span></td><td>{e(r['reviewer'] or 'Awaiting review')}</td></tr>" for r in rows)
                body=f"""<div class='split'><div class='card'><h2>Apply for leave</h2><p class='muted'>Submitting creates an immediate admin notification.</p><form method='post' action='/leave'><div class='cols'><div><label>From</label><input type='date' name='date_from' required></div><div><label>To</label><input type='date' name='date_to' required></div></div><label>Leave type</label><select name='leave_type'><option>Annual leave</option><option>Sick leave</option><option>Personal leave</option></select><label>Reason</label><input name='reason' maxlength='120' required><p><button class='primary'>Submit leave request</button></p></form></div><div class='card'><h2>Approval flow</h2><p class='muted'>1. Submit your request.<br><br>2. Admin receives a notification.<br><br>3. The decision appears below.</p></div></div><div class='card table section'><h2>My requests</h2><table><tr><th>Dates</th><th>Type</th><th>Reason</th><th>Status</th><th>Reviewer</th></tr>{trs}</table></div>"""
            ...
            if path=="/admin/leaves":
                ...
                trs="".join(f"<tr><td>{e(r['name'])}<div class='muted'>{e(r['employee_id'])}</div></td><td>{e(r['date_from'])} → {e(r['date_to'])}</td><td>{e(r['leave_type'])}</td><td>{e(r['reason'])}</td><td><span class='status {r['status'].lower()}'>{e(r['status'])}</span></td><td>{self.leave_actions(r)}</td></tr>" for r in rows)
                pending=sum(r["status"]=="Pending" for r in rows)
                return self.send_html(page("Leave approvals",f"<div class='flash'>🔔 {pending} new leave notifications</div><div class='card table'><table><tr><th>Employee</th><th>Dates</th><th>Type</th><th>Reason</th><th>Status</th><th>Decision</th></tr>{trs}</table></div>",user,"leaves"))
